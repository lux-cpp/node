...
// Where the archive publishes a chain: the Go node's own routes (server.Chain in
// luxfi/node), `/v1/chain/<alias>` with the EVM's JSON-RPC under `/rpc`. The
// `/v1/bc/...` spelling this used to send was dropped from luxd in v1.36.179, and
// a proxy still aiming there turns every historical query into a 404.
//
// The archive is a node of this node's OWN network, so this node's chain is that
// archive's EVM — which Go registers as C whatever the network is called. P and X
// exist only on Lux, the one network that owns them. Whether this node may answer
// for an alias at all was settled before this is asked (see Network); an alias
// the archive does not publish is not proxied, because a guessed path returns a
// 404 wearing the costume of an outage.
std::string archive_path(const Network& net, const std::string& alias) {
    if (std::find(net.served.begin(), net.served.end(), alias) != net.served.end()) return "/v1/chain/C/rpc";
    if (alias == "p" && net.owns(alias)) return "/v1/chain/P";
    if (alias == "x" && net.owns(alias)) return "/v1/chain/X";
    return "";
}
...
void Rpc::answer(int fd) {
    ...
    if (!kept) {
        ...
        // node serves P and X.
        if (!archive_rpc_.empty()) {
            const Proxied proxied = proxy_to_archive(archive_rpc_, archive_path(net_, alias), body);
            if (!proxied.empty()) {
...
Rpc::Json Rpc::dispatch(const std::string& alias, const Json& req) {
    ...
    if (it == table.end()) {
        if (!archive_rpc_.empty()) {
            const Proxied proxied = proxy_to_archive(archive_rpc_, archive_path(net_, alias), req.dump());
