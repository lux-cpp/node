...
int main() {
    ...
    {
        // The one relay a sovereign L1 has: its OWN chain's history. The archive
        // is a node of the same network, and Go registers that network's EVM as
        // C, so a method this frontier does not keep goes there rather than
        // failing — under the node's own name and under its chain id alike.
        Archive  archive(200, "OK", R"({"jsonrpc":"2.0","id":1,"result":"0x2a"})", false);
        Frontier node(archive.url(), 200200);
        const char* kHistory = R"({"jsonrpc":"2.0","id":1,"method":"eth_getBalance","params":["0x0","0x1"]})";
        for (const char* path : {"/v1/chain/zoo", "/v1/chain/200200/rpc"}) {
            const Reply r = call(node.port(), path, kHistory);
            check(r.status == 200 && r.body.find("0x2a") != std::string::npos,
                  std::string("a Zoo node relays its own history via ") + path);
        }
        check(archive.served() == 2, "…to its own network's archive, once per question");
    }
